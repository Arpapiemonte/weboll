...
<template>
  ...
  <div class="container-fluid">
    ...
    >
      ...
      >
        <a
          ...
          :href="'/api/w06/pdf/' + autostrada.id_w06"
          target="_blank"
          role="button"
        >
          <img
    ...
    <div class="row mb-3">
      <h1>Bollettino A6 {{ autostrada.id_w06 }}</h1>
    </div>
    <div class="row">
      ...
      <div class="col-md-2 mb-3">
        <label for="data_validita">Data scadenza
          ...
          <!--<Datepicker
            ...
            format="dd/MM/yyyy"
            auto-apply
            @update:model-value="saveW06(autostrada.data_validita, autostrada.id_w06, 'data_validita')"
          />-->
        </label>
      </div>
      <div class="col-md-2 mb-3">
      ...
      <div v-if="ready">
        ...
        >
          <div class="col-md-12">
            ...
            >
              <li
                v-for="(value, key) in autostrada.w06_data"
                :key="key"
                class="nav-item"
                role="presentation"
              >
        ...
        <div class="row">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td class="text-center">
                        <ClassSelect 
                          :data="placeholder"
                          :classes-value="configA6.precipitation"
                          :validity="false"
                    ...
                    <tr
                      v-for="tratto in autostrada.w06_data[selected_time_layout]"
                      :key="tratto.id_w06"
                    >
                      <td
                        class="text-center"
                      ...
                      <td class="text-center">
                        <select
                          ...
                          class="form-select col"
                          @change="setMeasure(tratto.id_w06_data, 'sky_condition')"
                        >
                          <option
                            v-for="(d, j) in icons"
                            :key="j"
                      ...
                      <td class="text-center">
                        <input
                          ...
                          :disabled="readonly"
                          @change="setMeasure(tratto.id_w06_data, 'cumulated_snow')"
                        >
                      </td>
                      <td class="text-center">
                        <SnowInput 
                          :measure="tratto.freezing_level"
                          :id-w06-data="tratto.id_w06_data"
                          :campo="'freezing_level'"
                          :refresher="refresher"
                          :readonly="readonly"
                          @setmeasure="saveW06Data"
                        />
                      </td>
                      <td class="text-center">
                        <SnowInput 
                      ...
                      <td class="text-center">
                        <input 
                          ...
                          type="checkbox"
                          :disabled="readonly"
                          @change="setCheckbox(tratto.id_w06_data, 'temperature_below_zero')"
                        >
                      </td>
                      <td class="text-center">
                        <input 
                          ...
                          type="checkbox"
                          :disabled="readonly"
                          @change="setCheckbox(tratto.id_w06_data, 'risk_freezing_rain')"
                        >
                      </td>
                    </tr>
                  </tbody>
          ...
          > 
            <div>
              <IconeMap
                :datatab="autostrada.w06_data[selected_time_layout]"
                :icons="icons"
                :venues-selected="venuesSelected"
                :show-autostrade="{
                  a26: false,
...
//import type { W06Processed, Icons } from "../types"
import { configA6 } from "../config";
...
const cumulatedSnowValidity = computed(() => {
  ...
  let validity = {}
  for(const tl in autostrada.value.w06_data){
    validity[tl] = {}
    autostrada.value.w06_data[tl].forEach(e => {
...
const skycond_validity = computed(() => {
  ...
  for(const tl in autostrada.value.w06_data){
    validity[tl] = {}
    autostrada.value.w06_data[tl].forEach(data => {
...
async function fetchData () {
  ...
  fetchAutostrada(autostrada_id.value).then(response => {
    ...
    if ('w06data_set' in data) {
      data.w06data_set.forEach((element) => {
        tmp[element.id_time_layouts].push(element)
      })
    }

    for(const tl in tmp){
      let tmparray: Array<W06Data> = []
...
function execute(action, reroute, message) {
  ...
  fetchAutostradaAction(action).then(response => {
    ...
    if (reroute) {
      router.push({ path: `/w06/${data.id_w06}`})
      autostrada_id.value = data.id_w06
      fetchData()
    } else {
      fetchData()
    }
...
function remove() {
  if (
    ...
    api.fetchBulletinDelete(autostrada_id.value, 'w06/bulletins', store).then(response => {
      if (response.ok) {
        toast.open(
          {
...
function setCheckbox(id_w06_data, campo){
  if(window.event && window.event.target !== null){
    let new_value = (window.event.target as HTMLInputElement).checked
    saveW06Data(id_w06_data, campo, new_value)
  }
}
function setMeasure(id_w06_data, campo){
  if(window.event && window.event.target !== null){
...
function saveW06Data(id_w06_data, campo, new_value){
  let w06data = autostrada.value.w06_data[selected_time_layout.value].find(e => e.id_w06_data === id_w06_data)
  let stack : Array<Object> = []

  if(campo === 'sky_condition'){
    if(icon_list.includes(new_value)){
      const payloadAzzeramento = {"id_key":"id_w06_data","id":w06data.id_w06_data,"value_key":"cumulated_snow","new_value": null}
      w06data["cumulated_snow"] = null
    ...
    for(const classe in pluv_classlist){
      if(pluv_classlist[classe].icons.includes(new_value)){
        ...
        const payloadPioggia = {"id_key":"id_w06_data","id":w06data.id_w06_data,"value_key":"precipitation_class","new_value": pluv_value }
        w06data["precipitation_class"] = pluv_value
  ...
  const payload = {"id_key":"id_w06_data","id":w06data.id_w06_data,"value_key":campo,"new_value": value}
  const payloadusername = {"id_key":"id_w06","id":autostrada.value.id_w06,"value_key":"username","new_value": store.state.username}
...
function saveAll(campo, new_value){
  ...
  let w06datas = autostrada.value.w06_data[selected_time_layout.value]
  let stack : Array<Object> = []

  w06datas.forEach(d => {
    const payload = {"id_key":"id_w06_data","id":d.id_w06_data,"value_key":campo,"new_value": new_value}
    d[campo] = new_value
    stack.push(payload)
    if(campo === 'sky_condition' && icon_list.includes(new_value)){
      const payloadAzzeramento = {"id_key":"id_w06_data","id":d.id_w06_data,"value_key":"cumulated_snow","new_value": null}
      d["cumulated_snow"] = null
      stack.push(payloadAzzeramento)
      for(const classe in pluv_classlist){
        if(pluv_classlist[classe].icons.includes(new_value)){
          let pluv_value = skycond_to_pluv[new_value]
          const payloadPioggia = {"id_key":"id_w06_data","id":d.id_w06_data,"value_key":"precipitation_class","new_value": pluv_value }
...
function saveW06(stack) {
  forceUpdate()
  bulkUpdateW06(stack).then((response) => {
...
function setIcons(id_sky_condition){  
  ...
  venuesSelected.value.forEach(venue => {
    let w06data = autostrada.value.w06_data[selected_time_layout.value].find(e => e.id_venue === parseInt(venue))
    const payload = {"id_key":"id_w06_data","id":w06data.id_w06_data,"value_key":"sky_condition","new_value": id_sky_condition}

    if(icon_list.includes(id_sky_condition) && w06data.cumulated_snow !== null){
      const payloadAzzeramento = {"id_key":"id_w06_data","id":w06data.id_w06_data,"value_key":"cumulated_snow","new_value": null}
      w06data["cumulated_snow"] = null
      stack.push(payloadAzzeramento)
    }
    ...
    w06data["sky_condition"] = id_sky_condition
    stack.push(payload)
  })
...
async function fetchAutostradaAction (action) {
  const response = await api.fetch_wrapper(
    store.state.access,
    `/api/w06/bulletins/${autostrada_id.value}/${action}/`
  )
  return response
}

async function fetchAutostrada (id) {
  const response = await fetch('/api/w06/bulletins/' + id + '/', {
    headers: {
...
async function bulkUpdateW06(payload) {
  if(store.state.access !== null){
    const response = await api.fetch_wrapper(
      store.state.access,
      `/api/w06/bulletins/bulk_update/`,
      {
        method: 'POST',
