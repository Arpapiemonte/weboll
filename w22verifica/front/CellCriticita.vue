...
<template>
  <td :style="`background-color: ${coloreHtml};`">
    <select
      :disabled="readonly"
      :value="idCriticita"
      @change="$emit('changeCriticita', $event.target.value, area.id_w22verifica_data, area.id_w22_zone.id_w22_zone, campo)"
    >
      <option
...
export default {
  ...
  beforeMount() {
    Object.keys(this.criticita).forEach(id => {
      this.criticitaRe[this.criticita[id].id_w22_criticita] = this.criticita[id].colore_html
    })
  },
}
</script>
