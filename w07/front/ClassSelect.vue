...
<template>
  <select
    :name="String(data.id_w07_data + campo)"
    :value="data[campo]"
    class="form-select"
...
const emit = defineEmits<{
  (e: "setClass", id_w07_data: number, campo: string, new_value: number): void
}>()

function onChange(e: Event) {
  const target = e.target as HTMLInputElement
  const new_value = parseInt(target.value)
  emit("setClass", props.data.id_w07_data, props.campo, new_value)
}
